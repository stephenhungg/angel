/**
 * voiceConfig.ts — deterministic mapping from numeric trait vector → animalese
 * params. Same vector = same voice every time.
 *
 * Used:
 *  - swipe card hover: 0.5s preview using the candidate's tags → VoiceConfig
 *  - reveal screen: her first line + naming-beat response uses the user's
 *    chosen attractor's config
 *  - electron orchestrator: every utterance uses the same config (claim
 *    payload includes it)
 *
 * Ported verbatim from web/lib/voice-config.ts.
 */
...
export function voiceConfigFromTraits(
  ...
  void s; // sophistication enters via cluster + vowel pickers, not the params below
...
/* ------------------------------------------------------------------ */
/* derived categorical traits — from /api/embed (web), inlined here    */
/* so the renderer can compute traits without a server roundtrip       */
/* ------------------------------------------------------------------ */

import type { DispositionArchetype, StyleArchetype } from '@angel/shared';

export function dispositionFromTraits(t: NumericTraits): DispositionArchetype {
...
export function styleFromTraits(t: NumericTraits): StyleArchetype {
